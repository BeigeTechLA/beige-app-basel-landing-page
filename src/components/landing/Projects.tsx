...
const CARD_WIDTH_VW = 32;   // ~2.5 cards in viewport
const GAP_PX = 20;
const CLONES = 2;
...
export const Projects = () => {
  ...
  React.useEffect(() => {
    if (index >= PROJECTS.length + CLONES) {
      querySelectorNoAnim(() => setIndex(CLONES));
    }
    if (index < CLONES) {
      querySelectorNoAnim(() => setIndex(PROJECTS.length + CLONES - 1));
    }
  }, [index]);

  const querySelectorNoAnim = (cb: () => void) => {
    setTimeout(cb, 350);
  };

  return (
    <section className="py-20 md:py-32 bg-[#010101] border-t border-white/5 relative overflow-hidden">
      <Container>
